...
堆排序就是利用堆的性质，将无序数组插入堆中，重复删除堆中最大值，满足排序需求。

堆排序看起来很繁琐，它到底能用来干什么？在数据处理中，往往数据量很大，甚至无法全部读进内存，无法排序。如果你需要从 10 亿个元素中取出 TOP 10，显然不柯南构建一个 10 亿规模的数组，这时候就需要堆排序了。