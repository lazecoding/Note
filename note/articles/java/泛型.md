...
上面这个例子，我们成功利用反射在运行期给 List 添加 String 类型参数，表明泛型只在编译期有效，为什么运行期失效了呢，这是因为泛型的擦除概念，通俗点来说就是泛型的信息不会进行运行阶段。
...
public class Consumers {
    public static void main(String[] args) {
        ...
        Cat cat = new Cat();
        Dog dog = new Dog();
        System.out.println("我想要一只 "+shop.get(cat).getName()); // 我想要一只 cat
        System.out.println("我想要一只 "+shop.get(dog).getName()); // 我想要一只 dog
        System.out.println("我想要一只 "+shop.get(whiteCat).getName());  // 我想要一只 white cat
    }
}
````
...
public class Consumers {
    public static void main(String[] args) {
        ...
        System.out.println("我想要一只 " + catShop.get(whiteCat).getName());  // 我想要一只 white cat
        System.out.println("我是一只猫吗? " + catShop.get(whiteCat).isCat()); // 我是一只猫吗? true
    }
}
````
