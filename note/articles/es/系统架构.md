...
Dedicated master-eligible 节点仅具有 master role，从而专注于管理集群(但是依旧可以充当协调节点，并将搜索和索引请求从客户端路由到数据节点)。
...
数据节点包含包含你已建立索引的文档的分片。 数据节点处理与数据相关的操作，这些操作是 I/O、内存和 CPU 密集型的，例如 CRUD，搜索和聚合。
